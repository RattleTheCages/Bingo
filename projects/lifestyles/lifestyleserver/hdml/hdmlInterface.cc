...
void hdmlInterface_o::init()  {
    ...
    TransactionServerPath = "http://204.131.41.3:4111/";
...
int hdmlInterface_o::parse(pdaInput_o& pdaInput)  {
    ...
    if(ps2.contains("x-UP-SubNo"))  ps2.upcut("x-UP-SubNo:");
    else  ps2.upcut("x-up-subno:");
    ps2.cut('\n');
    if(ps2.charat(ps2.length()-1) == 13)  ps2.cut(ps2.length()-1);
    ps2.boxears();
